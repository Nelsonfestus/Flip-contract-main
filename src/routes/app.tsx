...
const NAV_LINKS = [
  { to: '/app' as const, label: 'Hub', icon: Home },
...
function HubPage() {
  ...
  return (
    ...
    <div style={{ minHeight: '100dvh', background: 'transparent' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0, marginLeft: 'auto' }}>
            <Link
              ...
              style={{
                ...
                border: '1px solid rgba(92,184,133,0.3)',
                color: '#5cb885', cursor: 'pointer',
                fontSize: 12, fontFamily: "'DM Sans', sans-serif",
                fontWeight: 600, transition: 'all 0.2s', whiteSpace: 'nowrap', textDecoration: 'none'
              }}
              onMouseEnter={e => { e.currentTarget.style.background = 'rgba(92,184,133,0.2)'; }}
              onMouseLeave={e => { e.currentTarget.style.background = 'rgba(92,184,133,0.1)'; }}
            >
              <Users size={12} /> CRM
            </Link>
            <GlobalSearch />
            <span className={`badge badge-green nav-member-badge`} style={{ fontSize: 11, display: 'flex', alignItems: 'center', gap: 4 }}>
        ...
        {mobileMenuOpen && (
          <div className="animate-slide-down" role="menu" style={{
            ...
            {user ? (
              ...
              <>
                <div style={{ height: 1, background: '#3d4e65', margin: '8px 24px' }} />
                <Link to="/login" role="menuitem" onClick={() => setMobileMenuOpen(false)} style={{
                  display: 'flex', alignItems: 'center', gap: 10, width: '100%', padding: '14px 24px',
      ...
      {/* Demo Booking Modal */}
      {showDemo && <DemoBookingForm onClose={() => setShowDemo(false)} />}
            ...
            return (
              <Link
                ...
                onClick={(e) => {
                  if (isLockedForFree) {
                    e.preventDefault()
                    if (confirm(`The "${cat.title}" section is reserved for PRO members. Would you like to book a live demo to see how it works?`)) {
                      setShowDemo(true)
                    }
                  }
                }}
